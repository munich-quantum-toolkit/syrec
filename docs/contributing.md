...
Thank you for your interest in contributing to MQT SyReC Synthesizer! This
document outlines the development guidelines and how to contribute.

We use GitHub to [host code](https://github.com/munich-quantum-toolkit/syrec),
to [track issues and feature requests][issues], as well as accept
[pull requests](https://github.com/munich-quantum-toolkit/syrec/pulls). See
...
- 🐛 Report bugs:
  ...
  <https://github.com/munich-quantum-toolkit/syrec/issues>. Include steps to
...
- 💡 Propose features:
  ...
  <https://github.com/munich-quantum-toolkit/syrec/issues>. Describe the
...
- 🙌 Community support:
  ...
  <https://github.com/munich-quantum-toolkit/syrec/discussions>.
...
which contains context and instructions to help the agent work on MQT SyReC
Synthesizer. For Claude Code, create a symlink with
...
[GitHub Actions page](https://github.com/munich-quantum-toolkit/syrec/actions/workflows/ci.yml).
...
{code}`-DBUILD_MQT_SYREC SYNTHESIZER_TESTS=OFF` to the CMake configure step.
:::

Our CI pipeline on GitHub also collects code coverage information and uploads it
to [Codecov](https://codecov.io/gh/munich-quantum-toolkit/syrec). Our goal is to
...
{code}`-DBUILD_MQT_SYREC SYNTHESIZER_BINDINGS=ON` to the CMake configure step.
...
The Python package itself lives in the {code}`python/mqt/syrec` directory.

The package lives in the {code}`src/mqt/syrec` directory.
...
in the {code}`python/mqt/syrec` directory. These stub files are auto-generated.
...
a breaking change for MQT SyReC Synthesizer. If you are sure that the dependency
update does not introduce any breaking changes for MQT SyReC Synthesizer, you
can remove the {code}`minor` or {code}`major` label from the PR. This will
ensure that the respective PR does not influence the type of an upcoming
release.

:::

Once everything is in order, navigate to the
[Releases page](https://github.com/munich-quantum-toolkit/syrec/releases) on
