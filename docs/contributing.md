...
Thank you for your interest in contributing to MQT SyReC Synthesizer! This document
outlines the development guidelines and how to contribute.

We use GitHub to
[host code](https://github.com/munich-quantum-toolkit/syrec), to
...
which contains context and instructions to help the agent work on MQT SyReC Synthesizer.
...
to [Codecov](https://codecov.io/gh/munich-quantum-toolkit/syrec). Our goal is
...
The Python package itself lives in the {code}`python/mqt/syrec`
directory.
...
stub files in the {code}`python/mqt/syrec` directory. These stub files
...
a breaking change for MQT SyReC Synthesizer. If you are sure that the dependency update
does not introduce any breaking changes for MQT SyReC Synthesizer, you can remove the
