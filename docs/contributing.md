...
[pull requests](https://github.com/munich-quantum-toolkit/syrec/pulls).
...
- 🐛 Report bugs:
  ...
  <https://github.com/munich-quantum-toolkit/syrec/issues>.
...
you can pass {code}`-DBUILD_MQT_SYREC SYNTHESIZER_TESTS=OFF` to the CMake
configure step.
:::

Our CI pipeline on GitHub also collects code coverage information
and uploads it to
[Codecov](https://codecov.io/gh/munich-quantum-toolkit/syrec).
...
pass {code}`-DBUILD_MQT_SYREC SYNTHESIZER_BINDINGS=ON` to the CMake configure
step.
...
{code}`python/mqt/syrec` directory.
...
for MQT SyReC Synthesizer.
If you are sure that the dependency update does not introduce any breaking
changes for MQT SyReC Synthesizer, you can remove the {code}`minor` or {code}`major`
label from the PR.
