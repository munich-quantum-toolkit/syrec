...
- 🐛 Report bugs:
  ...
  Use the _🐛 Bug report_ template at <https://github.com/munich-quantum-toolkit/syrec/issues>.
...
- 💡 Propose features:
  ...
  Use the _✨ Feature request_ template at <https://github.com/munich-quantum-toolkit/syrec/issues>.
...
It will also collect code coverage information and upload it to [Codecov](https://codecov.io/gh/munich-quantum-toolkit/syrec).
...
:::{tip}
If you want to disable configuring and building the C++ tests, you can pass {code}`-DBUILD_MQT_SYREC SYNTHESIZER_TESTS=OFF` to the CMake configure step.
:::
...
This does not mean that the dependency update itself is a breaking change for MQT SyReC Synthesizer.
If you are sure that the dependency update does not introduce any breaking changes for MQT SyReC Synthesizer, you can remove the {code}`minor` or {code}`major` label from the PR.
This will ensure that the respective PR does not influence the type of an upcoming release.

:::

Once everything is in order, navigate to the [Releases page](https://github.com/munich-quantum-toolkit/syrec/releases) on GitHub, edit the release draft if necessary, and publish the release.
