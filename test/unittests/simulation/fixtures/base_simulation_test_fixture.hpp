...
protected:
    ...
    static void assertSimulationResultForStateMatchesExpectedOne(const syrec::AnnotatableQuantumComputation& annotatableQuantumComputation, const syrec::NBitValuesContainer& inputState, const syrec::NBitValuesContainer& expectedOutputState, const std::set<qc::Qubit>& nonAncillaryQubitsLookup) {
        ...
        if (inputState.size() == 0 || nonAncillaryQubitsLookup.empty()) {
            return;
        }
    ...
    static void loadNBitValuesContainerFromString(syrec::NBitValuesContainer& container, const std::string& stringifiedBinaryState) {
        ...
        ASSERT_GE(container.size(), stringifiedBinaryState.size()) << "Expected size of NBitValues container must be equal or larger than the stringified binary state size";
